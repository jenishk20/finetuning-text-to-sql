...
Usage:
    ...
    python -m src.bird_pipeline --list-models

    # Run with specific model via OpenRouter
    python -m src.bird_pipeline --provider openrouter --model deepseek-v3
    python -m src.bird_pipeline --provider openrouter --model gpt-4o --limit 10

    # Run with Grok (xAI)
    python -m src.bird_pipeline --provider xai --model grok-4-1-fast-reasoning

    # Resume a previous run
    python -m src.bird_pipeline --resume results/bird_deepseek-v3_XXXX.json
"""
...
from src.schema_loader import get_schema_from_sqlite
